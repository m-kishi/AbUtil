SELECT
  ...
  , SUM(CASE WHEN type NOT IN ('収入', '特入', '特出', '秘密入', '秘密出', '投資', '利益', '損益') THEN cost ELSE 0 END) AS ttal
  , SUM(CASE WHEN type = '収入' THEN cost WHEN type NOT IN('収入', '特入', '特出', '秘密入', '秘密出', '投資', '利益', '損益') THEN -cost ELSE 0 END) AS blnc
FROM
    expenses
GROUP BY
    year, month
